from sympy import *
from hashlib import md5
from Crypto.Cipher import AES
from Crypto.Util.Padding import pad
from random import randint, randbytes,seed
from Crypto.Util.number import bytes_to_long

dummyflag = "lol"
FLAG = b'HTB{dummyflag}'
class Shamir:
    def __init__(self, prime, k, n):
        self.p = prime
        self.secret = 16 # randint(1,self.p-1)
        self.k = k
        self.n = n
        self.coeffs = [self.secret, 93526756371754197321930622219489764824]
        self.x_vals = []
        self.y_vals = []

    def next_coeff(self, val):
        # breakpoint()
        return int(md5(val.to_bytes(32, byteorder="big")).hexdigest(),16)

    def calc_coeffs(self):
        for i in range(2, self.n + 1):
            self.coeffs.append(self.next_coeff(self.coeffs[i-1]))


    """
    y = y + coeff * 21202245407317581090 ^ i
    """
    def calc_y(self, x):
        y = 0
        for i, coeff in enumerate(self.coeffs):        
            y += coeff * 21202245407317581090**i
        return y % self.p

    def create_pol(self):
        self.calc_coeffs()
        self.coeffs = self.coeffs[:self.k]
        x = 21202245407317581090
        self.x_vals.append(x)
        self.y_vals.append(self.calc_y(x))
        # breakpoint()

    def get_share(self):
        return self.x_vals[0], self.y_vals[0]



sss = Shamir(92434467187580489687, 10, 18)
sss.create_pol()
share = sss.get_share()


# breakpoint()


y = 0
for i in range(1, 10):        
    y += sss.coeffs[i] * 21202245407317581090 ** i

print(y)
# breakpoint()

# for i in range(0, sss.p):
#     y = y + i
#     print(f"{i}\r", end="", flush=True)
#     if y % sss.p == 11086299714260406068:
#         print(f"Found at i={i}")
#         breakpoint()
#         print(i)

y = y + 39612257993477957104
if y % sss.p == 11086299714260406068:
    print(f"Found key: {39612257993477957104}")
